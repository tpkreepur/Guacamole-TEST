...
associate_saml() {
    # Verify required parameters are present
    if [ -z "$SAML_IDP_METADATA_URL"]; then
        cat <<END
FATAL: Missing SAML_IDP_METADATA_URL.
-------------------------------------------------------------------------------
If using a SAML authentication, you must:

(a) Explicitly specify the connection to your Idp using the following
    environment variables:

    SAML_IDP_METADATA_URL     The metadata url of the Idp server.
...
END
    ...
    set_optional_property "saml-idp-url"  "$SAML_GROUP_ATTRIBUTE"
    ...
    # Add required .jar files to GUACAMOLE_EXT
    ln -s /opt/guacamole/saml/guacamole-auth-*.jar "$GUACAMOLE_EXT"
}
